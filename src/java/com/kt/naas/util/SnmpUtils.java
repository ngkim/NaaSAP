...
public class SnmpUtils {
	 ...
	 * @param ip agent IP주소
	 * @param port agent 포트번호
	 * @param community agent community
	 * @return SNMP 연결을 위한 Target
	 */
	public static Target getCommunityTarget(String ip, int port, String community)
	{
	...
	/**
	 * VariableBinding 의 value part 를 문자열로 변환한다.
	 * SNMP4J 라이브러리의 toString() 에서는 DisplayString이 아닌 경우 HEXA 코드로 리턴된다.
	 * 본 함수를 이용할 경우 DisplayString이 아니더라도 원래의 String을 리턴한다.
	 * 비 영어권 문자열이 포함된 경우 의도하지 않게 hexa로 출력되는 문제를 해결하기 위해 사용한다.
	 * 또한 VariableBinding의 value part가 SNMP NULL 인경우 null String을 리턴한다. 
	 * @param vb 데이터를 추출한 VariableBinding 
	 ...
	 * 또한 VariableBinding의 value part가 SNMP NULL 인경우 null String을 리턴한다. 
	 * @param vb 데이터를 추출한 VariableBinding 
	 * @param defaultValue value part가 snmp null 인 경우 리턴될 기본값
	...
	public static String getStringValue(VariableBinding vb, String defaultValue)
	{
		...
		if (v == null || v.getSyntax() == SMIConstants.SYNTAX_NULL || v.getSyntax() >= 128) return defaultValue;
	...
	/**
	 * VariableBinding 의 value part 를 int 값으로 변환한다.
	 * value part 가 snmp null 인 경우 defaultValue를 리턴한다. 
	 * @param vb 데이터를 추출한 VariableBinding
	 * @param defaultValue
	 * @return int 값
	 */
	 ...
	 */
	/***
	public static double getDoubleValue(VariableBinding vb)
	{
		return getDoubleValue(vb, GlobalConstants.NULL_DOUBLE);
	}
	...
	/***
	public static double getDoubleValue(VariableBinding vb, long defaultValue)
	{
		if (vb == null) return defaultValue;
		Variable v = vb.getVariable();
	...
	*/
	...
	public static String getHexString(VariableBinding vb)
	{
		...
		if (v == null || v.getSyntax() == SMIConstants.SYNTAX_NULL || v.getSyntax() >= 128) return null;
	...
	public static List<OID> getInstanceOids(Map<OID, Map<String, VariableBinding>> resultMap)
	{
